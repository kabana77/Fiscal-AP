...
type
  TValidasiBonBBBenangFrm = class(TForm)
    Panel1: TPanel;
    ...
    QDetailQTY_STOK_BISFA: TFloatField;
    QDetailQTY_BON_BISFA: TFloatField;
    QDetailQTY_VALID_BISFA: TFloatField;
...
var
  ValidasiBonBBBenangFrm: TValidasiBonBBBenangFrm;
...
procedure TValidasiBonBBBenangFrm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=caFree;
  ValidasiBonBBBenangFrm:=Nil;
end;

procedure TValidasiBonBBBenangFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TValidasiBonBBBenangFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TValidasiBonBBBenangFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TValidasiBonBBBenangFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TValidasiBonBBBenangFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
...
procedure TValidasiBonBBBenangFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
...
procedure TValidasiBonBBBenangFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.QDetailNewRecord(DataSet: TDataSet);
begin
  QDetailOPR_INSERT.AsString:=DMFrm.QUserNAMA_USER.AsString;
  QDetailQTY_BISFA.AsFloat:=QDetailQTY.AsFloat*1.0198;
end;

procedure TValidasiBonBBBenangFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TValidasiBonBBBenangFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiBonBBBenangFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TValidasiBonBBBenangFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TValidasiBonBBBenangFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TValidasiBonBBBenangFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TValidasiBonBBBenangFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TValidasiBonBBBenangFrm.ECariChange(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TValidasiBonBBBenangFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TValidasiBonBBBenangFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TValidasiBonBBBenangFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TValidasiBonBBBenangFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TValidasiBonBBBenangFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TValidasiBonBBBenangFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiISPOST.AsString='1' then
  begin

    if QTransaksiJNS_KOREKSI.AsString='DIJUAL' then
       QRLTitle2.Caption:='SURAT JALAN'
    else
    ...
    if QTransaksiKD_TRANSAKSI.AsString='V01' then
       QRLTitle2.Caption:='VALIDASI BON BAHAN BAKU';
...
procedure TValidasiBonBBBenangFrm.FormCloseQuery(Sender: TObject;
...
procedure TValidasiBonBBBenangFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TValidasiBonBBBenangFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TValidasiBonBBBenangFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TValidasiBonBBBenangFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TValidasiBonBBBenangFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TValidasiBonBBBenangFrm.RadioGroup1Click(Sender: TObject);
begin
  case RadioGroup1.ItemIndex of
  0 : vRecord.Value:=40;
  1 : vRecord.Value:=20;
...
procedure TValidasiBonBBBenangFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TValidasiBonBBBenangFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TValidasiBonBBBenangFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TValidasiBonBBBenangFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TValidasiBonBBBenangFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TValidasiBonBBBenangFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TValidasiBonBBBenangFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TValidasiBonBBBenangFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TValidasiBonBBBenangFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TValidasiBonBBBenangFrm.QLookLokasiBeforeOpen(DataSet: TDataSet);
begin
  QLookLokasi.SetVariable('kd_jns_item',QDetailKD_JNS_ITEM.AsString);
end;

procedure TValidasiBonBBBenangFrm.LookLokasiCloseUp(Sender: TObject;
...
procedure TValidasiBonBBBenangFrm.LookLokasiEnter(Sender: TObject);
begin
  QLookLokasi.Close;
  QLookLokasi.Open;
end;

procedure TValidasiBonBBBenangFrm.BtnFormulasiClick(Sender: TObject);
begin
  if (QTransaksiNO_REFF.AsString='')  then
    ShowMessage('No. Bukti Bon harus diisi !')
    else
    begin
   // label1.caption:=copy(QTransaksiNO_REFF.AsString,1,3);
...
procedure TValidasiBonBBBenangFrm.QBrowseCalcFields(DataSet: TDataSet);
begin
  QBrowseQTY2.AsFloat:=QBrowseQTY.AsFloat/cball2kg;
end;

procedure TValidasiBonBBBenangFrm.QDetailCalcFields(DataSet: TDataSet);
begin
...
procedure TValidasiBonBBBenangFrm.LookKoreksiEnter(Sender: TObject);
begin
  QJnsKoreksi.Open;
end;

procedure TValidasiBonBBBenangFrm.QDetailQTYChange(Sender: TField);
begin
  QDetailQTY_BISFA.AsFloat:=QDetailQTY.AsFloat*1.0198;
end;
