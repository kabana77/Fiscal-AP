...
type
  TPONBBFrm = class(TForm)
    ...
    QRLabel98: TQRLabel;
    QRGroup2: TQRGroup;
    QBrowse4NO_REG_PO: TIntegerField;
    QBrowse4TANGGAL: TDateTimeField;
    QBrowse4PPN: TStringField;
    QBrowse4ISPJK: TStringField;
    QBrowse4STATUS: TStringField;
    ...
    QBrowse4NO_PV: TStringField;
    QBrowse4AMOUNT: TFloatField;
    QBrowse4BY_LAIN2: TFloatField;
    QBrowse4DPP: TFloatField;
    QBrowse4NIL_PPN: TFloatField;
    QBrowse4REKANAN2: TStringField;
...
procedure TPONBBFrm.FormClose(Sender: TObject;
...
procedure TPONBBFrm.FormCreate(Sender: TObject);
begin
  DMFrm.QLookSubBagian.Close;
  QLookItem.Close;
end;

procedure TPONBBFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TPONBBFrm.wwDBGrid2TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
  if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TPONBBFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TPONBBFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TPONBBFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
  wwDBGrid2.SetFocus;
  TabSheet1.TabVisible:=QTransaksiISPOST.AsString='1';
end;

procedure TPONBBFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TPONBBFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
{azmi}
...
procedure TPONBBFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
{azmi}
...
procedure TPONBBFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TPONBBFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TPONBBFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TPONBBFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TPONBBFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TPONBBFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TPONBBFrm.QLookItemBeforeOpen(DataSet: TDataSet);
begin
  QLookItem.SetVariable('kd_jns_kebutuhan',vkd_jns_kebutuhan);
end;

procedure TPONBBFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TPONBBFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TPONBBFrm.ECariChange(Sender: TObject);
begin
...
procedure TPONBBFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TPONBBFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TPONBBFrm.FormShow(Sender: TObject);
begin
...
procedure TPONBBFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
  QBrowse.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TPONBBFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  vnomer:=0;
...
procedure TPONBBFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TPONBBFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TPONBBFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TPONBBFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QCaraBayar.close;
  QCaraBayar.Open;
...
procedure TPONBBFrm.FormCloseQuery(Sender: TObject;
...
procedure TPONBBFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TPONBBFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TPONBBFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vrecord_ke);
...
procedure TPONBBFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TPONBBFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TPONBBFrm.ColumnHeaderBand2BeforePrint(
...
procedure TPONBBFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TPONBBFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TPONBBFrm.vHeightAfterUpClick(Sender: TObject);
begin
  case PageControl1.ActivePageIndex of
...
procedure TPONBBFrm.BtnExportClick(Sender: TObject);
begin
{  if QBrowse.Active then
  begin
...
procedure TPONBBFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TPONBBFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TPONBBFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TPONBBFrm.QDetailBeforePost(DataSet: TDataSet);
begin
  QDetailOPR_UPDATE.AsString:=DMFrm.QUserNAMA_USER.AsString;
end;

procedure TPONBBFrm.BtnFormulasiClick(Sender: TObject);
{azmi}
var
nonota : integer;
valjns_pp : string;
...
{azmi}
  if (ENoNota.Text='') and (QTransaksiKD_REKANAN.AsString='') and (QTransaksiMU.AsString='') or (valjns_pp<>vkdjns_pp) then
    ShowMessage('No. PP, REKANAN, Cara Bayar dan Mata Uang harus diisi !')
    else
    begin
        if (QTransaksiISPOST.AsString='0') then
        begin
...
procedure TPONBBFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TPONBBFrm.LookSuplierCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
var
  vkota : string;
begin
...
procedure TPONBBFrm.LookSuplierEnter(Sender: TObject);
begin
  QSuplier.Open;
end;

procedure TPONBBFrm.LookSuplierUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  QSuplier.Close;
  QSuplier.Open;
...
procedure TPONBBFrm.LookMUEnter(Sender: TObject);
begin
  DMFrm.QMU.Open;
end;

procedure TPONBBFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TPONBBFrm.wwDBGrid2UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
  QTotal.SetVariable('no_reg_po',QTransaksiNO_REG_PO.AsInteger);
  QTotal.Open;
...
procedure TPONBBFrm.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
{azmi}
  ...
  begin
     ...
     QRLTerbilang.Caption:={'Terbilang : }'#'+DMFrm.MyEUR.HasilKonversi+'#';
...
procedure TPONBBFrm.SummaryBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
 { if (QTransaksiDIKIRIM_KE.AsString<>DMFrm.QPerusahaanALAMAT.AsString) then
    QRLKirim.Caption:='Mohon dikirim ke : '+QTransaksiDIKIRIM_KE.AsString
...
procedure TPONBBFrm.QuickRep2BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vdpp:=0;
  vppn:=0;
...
procedure TPONBBFrm.QBrowseDetailBeforeOpen(DataSet: TDataSet);
begin
//  QBrowseDetail.SetVariable('no_reg_po',QBrowseNO_REG_PO.AsInteger);
end;

procedure TPONBBFrm.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vnomer);
...
procedure TPONBBFrm.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vnomer2);
...
procedure TPONBBFrm.QSuplierBeforeOpen(DataSet: TDataSet);
begin
...
procedure TPONBBFrm.wwDBGrid1URLOpen(Sender: TObject; var URLLink: String;
  Field: TField; var UseDefault: Boolean);
var
  vstatus : String;
begin
...
{  vstatus:='CLOSE';
  if InputQuery('Ubah Status PO','CLOSE/CANCEL/OPEN :',vstatus) then
  begin
     QUpdateStatus.Close;
     QUpdateStatus.SetVariable('no_reg_po',QBrowseNO_REG_PO.AsInteger);
     QUpdateStatus.SetVariable('status',vstatus);
...
procedure TPONBBFrm.Btn_filterClick(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
...
procedure TPONBBFrm.dbcfield2Enter(Sender: TObject);
var
  i : Word;
begin
...
procedure TPONBBFrm.Vtglawal2Change(Sender: TObject);
begin
 vTglAkhir2.Date:=Trunc(EndOfTheMonth(VTglAwal2.Date));
end;

procedure TPONBBFrm.Vtglakhir2Change(Sender: TObject);
begin
...
procedure TPONBBFrm.cbotomatis2Click(Sender: TObject);
begin
...
procedure TPONBBFrm.TabSheet3Show(Sender: TObject);
begin
...
procedure TPONBBFrm.wwDBGrid3DblClick(Sender: TObject);
var
  myrvdDialog : TwwRecordViewDialog;
begin
  myrvdDialog:=TwwRecordViewDialog.Create(Nil);
...
procedure TPONBBFrm.wwDBGrid3TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
 if ((Sender as TwwDBGrid).ColumnByName(AFieldName).FieldName<>'') then
  begin
...
procedure TPONBBFrm.QBrowse2BeforeOpen(DataSet: TDataSet);
begin
  QBrowse2.SetVariable('kd_transaksi',QJnsTransaksiKD_TRANSAKSI.AsString);
end;

procedure TPONBBFrm.LookPPNEnter(Sender: TObject);
begin
...
procedure TPONBBFrm.LookMUCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  QTransaksiKURS.AsInteger:=1;  
...
procedure TPONBBFrm.BitBtn3Click(Sender: TObject);
begin
  Try
    DMFrm.QClosePO.Close;
    DMFrm.QClosePO.Execute;
    ShowMessage('Sukses !');
  except
       ShowMessage('Gagal !');
  end;
...
procedure TPONBBFrm.QDetailTGL_POChange(Sender: TField);
begin
  {if QTransaksiISPOST.AsString='1' then
  begin
      ShowMessage('Maaf, data sudah di-POSTING, tidak bisa diubah !');
...
procedure TPONBBFrm.LookPPNChange(Sender: TObject);
begin
{azmi
if lookppn.ItemIndex=-1 then
...
procedure TPONBBFrm.wwDBLookupComboDlg1CloseUp(Sender: TObject;
...
procedure TPONBBFrm.wwDBLookupComboDlg1Enter(Sender: TObject);
begin
{azmi}
  QUM.Close;
...
procedure TPONBBFrm.wwDBEdit7Exit(Sender: TObject);
begin
{azmi}
//    QTransaksiUM.AsFloat:=QTotalSUB_TOTAL.AsFloat*(QTransaksiPERSEN_UM.AsFloat/100);
              if QTransaksiPPN.AsString='EXC' then
...
procedure TPONBBFrm.wwCheckBox1Click(Sender: TObject);
begin
{iwan}
...
procedure TPONBBFrm.wwDBComboBox3CloseUp(Sender: TwwDBComboBox;
  Select: Boolean);
begin
  if wwDBComboBox3.ItemIndex = 0 then
    ...
    begin
      Label20.Refresh;
      wwDBSpinEdit1.Refresh;
      Label20.Caption:='Cash On Delivery';
      wwDBSpinEdit1.Visible:=false;
      QTransaksiNHARI.AsFloat:=0;
      QTransaksiSTATUS.AsString := 'CLOSE';
    end
    ...
    begin
      Label20.Refresh;
      wwDBSpinEdit1.Refresh;
      Label20.Caption:='Cash Before Delivery';
      wwDBSpinEdit1.Visible:=false;
      QTransaksiNHARI.AsFloat:=0;
      QTransaksiSTATUS.AsString := 'CLOSE';
    end
...
procedure TPONBBFrm.QCaraBayarBeforeQuery(Sender: TOracleDataSet);
begin
  QCaraBayar.SetVariable('no_reg_po', QTransaksiNO_REG_PO.AsString);
end;

procedure TPONBBFrm.QBrowse4AfterScroll(DataSet: TDataSet);
begin
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TPONBBFrm.BitBtn6Click(Sender: TObject);
begin
...
procedure TPONBBFrm.BitBtn4Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (QBrowse4.FieldByName(wwDBGrid4.Columns[i].FieldName).FieldKind=fkData) and ((UpperCase(wwDBGrid4.Columns[i].FieldName)<>'TANGGAL') and (UpperCase(wwDBGrid4.Columns[i].FieldName)<>'TGL_INSERT')) then
        begin
          if vpertama then
            begin
...
procedure TPONBBFrm.VTglAwal3Change(Sender: TObject);
begin
  vTglAkhir3.Date:=Trunc(EndOfTheMonth(VTglAwal3.Date));
end;

procedure TPONBBFrm.TabSheet4Show(Sender: TObject);
begin
...
procedure TPONBBFrm.QDetailBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TPONBBFrm.QRBand21BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if QTransaksiPPN.AsString='-' then
  begin
...
procedure TPONBBFrm.QRBand16BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  QCaraBayar.close;
  QCaraBayar.Open;
...
procedure TPONBBFrm.BitBtn7Click(Sender: TObject);
begin
