...
type
  TBKKFrm = class(TForm)
    ...
    QDetailNO_LINE: TStringField;
    QDetailNO_NOTA_PPH23: TStringField;
    QDetailPPH23: TFloatField;
    LookNotaPPH23: TwwDBLookupComboDlg;
    QNota_PPH23: TOracleDataSet;
    QNota_PPH23KD_REKANAN: TStringField;
    QNota_PPH23REKANAN: TStringField;
    QNota_PPH23NO_NOTA: TStringField;
    QNota_PPH23HT_PPH23: TFloatField;
    ...
    BitBtn4: TBitBtn;
    QRLabel31: TQRLabel;
    QNota_PPH23NO_REG_HUTANG: TFloatField;
    QNota_PPH23COA: TStringField;
    ...
    procedure LookBudgetUserButton2Click(Sender: TObject;
      LookupTable: TDataSet);
    procedure LookBudgetUserButton1Click(Sender: TObject;
      LookupTable: TDataSet);
  ...
  { procedure QuickRep1BeforePrint(Sender: TCustomQuickRep;
    ...
    procedure LookPerkiraanCloseUp(Sender: TObject; LookupTable,
      FillTable: TDataSet; modified: Boolean);
    procedure LookNotaPPH23Enter(Sender: TObject);
    procedure LookNotaPPH23CloseUp(Sender: TObject; LookupTable,
      FillTable: TDataSet; modified: Boolean);
  ...
  public
    { Public declarations }
    vkd_perk2, vmu, vpt,vjns, vrek_id, vrek_name : String;
    kd_filter : string;
    vhak_input : Boolean;
...
procedure TBKKFrm.FormClose(Sender: TObject;
...
procedure TBKKFrm.wwDBGrid1TitleButtonClick(Sender: TObject;
  AFieldName: String);
begin
...
procedure TBKKFrm.LookKonstruksiEnter(Sender: TObject);
begin
  DMFrm.QKonstruksi.Open;
end;

procedure TBKKFrm.cbTanggalClick(Sender: TObject);
begin
...
procedure TBKKFrm.QTransaksiAfterPost(DataSet: TDataSet);
begin
//  wwDBGrid2.SetFocus;
...
procedure TBKKFrm.wwDBGrid2Enter(Sender: TObject);
begin
...
procedure TBKKFrm.QTransaksiNewRecord(DataSet: TDataSet);
begin
  QSuplier.Close;
...
procedure TBKKFrm.QTransaksiBeforePost(DataSet: TDataSet);
begin
  ...
  begin
      ...
      begin
         ...
         // DMFrm.FNo_Nota_PMTX99.SetVariable('ppusat',QTransaksiISPUSAT.AsString);
...
procedure TBKKFrm.TabSheet2Show(Sender: TObject);
begin
...
procedure TBKKFrm.QTransaksiBeforeInsert(DataSet: TDataSet);
begin
...
procedure TBKKFrm.QTransaksiBeforeEdit(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBKKFrm.DBText1Click(Sender: TObject);
begin
  InputBox('Copy Nomer','Klik Kanan --> Copy : ',QTransaksiNO_NOTA.AsString);
end;

procedure TBKKFrm.DBText2Click(Sender: TObject);
var
  vno_reg_cari : String;
begin
...
procedure TBKKFrm.BitBtn2Click(Sender: TObject);
begin
  rvdTTD.Execute;
end;

procedure TBKKFrm.wwDBGrid2CalcCellColors(Sender: TObject;
...
procedure TBKKFrm.wwDBGrid1DblClick(Sender: TObject);
var
...
procedure TBKKFrm.ECariChange(Sender: TObject);
begin
...
procedure TBKKFrm.cbOtomatisClick(Sender: TObject);
begin
  if cbOtomatis.Checked then
  begin
...
procedure TBKKFrm.BitBtn1Click(Sender: TObject);
var
  i : word;
  vpertama : boolean;
...
procedure TBKKFrm.FormShow(Sender: TObject);
begin
  VTglAwal.Date:=Trunc(DMFrm.QDateTimeVNOW.AsDateTime);
  QJnsTransaksi.Open;
end;

procedure TBKKFrm.QBrowseBeforeOpen(DataSet: TDataSet);
begin
//QJnsTransaksi.Open;
...
procedure TBKKFrm.TitleBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  vmyberjalan:=0;
  vnomer:=0;
  QRLPeriode.Caption:=VTglAwal.Text+' S/D '+vTglAkhir.Text;
   QRDBText66.Caption:=dmfrm.QHakPerkiraannama_perkiraan.AsString;
   QRLTitle.Caption:='LAPORAN KAS HARIAN';
//  QRLTitle.Caption:=UpperCase(QJnsTransaksiNAMA_TRANSAKSI.AsString);
end;

procedure TBKKFrm.TabSheet1Show(Sender: TObject);
begin
...
procedure TBKKFrm.QBrowseAfterScroll(DataSet: TDataSet);
begin
  wwDBGrid2UpdateFooter(nil);
  LRecords.Caption:='Data ke '+IntToStr(DataSet.RecNo)+' dari '+IntToStr(DataSet.RecordCount)+' data';
end;

procedure TBKKFrm.TitleBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  {Lrek3.Caption:=dmfrm.QHakPerkiraannama_perkiraan.AsString;
  LRek4.Caption:=dmfrm.QHakPerkiraanKD_PERK.AsString;}
  Lrek3.Caption:=vrek_name;
  LRek4.Caption:=vrek_id;
  if QTransaksiISPOST.AsString='1' then
  begin
...
procedure TBKKFrm.FormCloseQuery(Sender: TObject;
...
procedure TBKKFrm.QDetailBeforeInsert(DataSet: TDataSet);
begin
  if not vhak_input then
  begin
      ShowMessage('Maaf, Anda tidak berhak merubah data !');
...
procedure TBKKFrm.QTransaksiAfterDelete(DataSet: TDataSet);
begin
  if QTransaksi.IsEmpty then
    TabSheet1.TabVisible:=True;
end;

procedure TBKKFrm.VTglAwalChange(Sender: TObject);
begin
  vTglAkhir.Date:=Trunc(EndOfTheMonth(VTglAwal.Date));
end;

procedure TBKKFrm.vTglAkhirChange(Sender: TObject);
begin
...
procedure TBKKFrm.RadioGroup1Click(Sender: TObject);
begin
...
procedure TBKKFrm.ColumnHeaderBand2BeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  vrecord_ke:=0;
end;

procedure TBKKFrm.wwDBGrid2DblClick(Sender: TObject);
var
...
procedure TBKKFrm.Button1Click(Sender: TObject);
begin
  wwDBGrid2DblClick(Nil);
end;

procedure TBKKFrm.vHeightAfterUpClick(Sender: TObject);
begin
...
procedure TBKKFrm.BtnExportClick(Sender: TObject);
begin
  if QBrowse.Active then
  begin
...
procedure TBKKFrm.DBText5Click(Sender: TObject);
var
  DC : HDC;
  ABitmap:TBitmap;
begin
...
procedure TBKKFrm.vOperandClick(Sender: TObject);
begin
  if vOperand.Caption='LIKE' then
    vOperand.Caption:='='
    else
...
procedure TBKKFrm.dbcFieldEnter(Sender: TObject);
var
  i : Word;
begin
...
procedure TBKKFrm.Label3Click(Sender: TObject);
begin
  DMFrm.QSisdur.Close;
...
procedure TBKKFrm.LookSuplierCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TBKKFrm.LookSuplierEnter(Sender: TObject);
begin
  QSuplier.Open;
end;

procedure TBKKFrm.LookSuplierUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
//arif 10062010
...
procedure TBKKFrm.LookMUEnter(Sender: TObject);
begin
  DMFrm.QMU.Open;
end;

procedure TBKKFrm.wwDBGrid2UpdateFooter(Sender: TObject);
begin
  QTotal.Close;
...
procedure TBKKFrm.QuickRep2BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vfirst_page:=True;
end;

procedure TBKKFrm.PageHeaderBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if vfirst_page then
  begin
...
{procedure TBKKFrm.wwDBLookupComboDlg1Enter(Sender: TObject);
begin
  QNota.Close;
...
{procedure TBKKFrm.QDetailBeforePost(DataSet: TDataSet);
begin
...
{procedure TBKKFrm.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if vfirst_page then
  begin
...
{procedure TBKKFrm.LookCashEnter(Sender: TObject);
begin
  DMFrm.QCash.Open;
end;
 }
...
procedure TBKKFrm.QRBand5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
inc(vrecord_ke);
...
procedure TBKKFrm.TabSheet3Show(Sender: TObject);
begin
  O_Notpost.Close;
  O_Notpost.Open;
end;

procedure TBKKFrm.LookProyekEnter(Sender: TObject);
begin
  DMFrm.QProyek.Open;
end;

procedure TBKKFrm.LookProyekUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
  DMFrm.QProyek.Close;
  DMFrm.QProyek.Open;
...
procedure TBKKFrm.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ...
  begin
    ...
    if QRLabel28.Caption = 'PV' then
       QRLabel28.Caption:='PAYMENT VOUCHER'
    else
      QRLabel28.Caption:=QTransaksiKD_TRANSAKSI.AsString;
...
procedure TBKKFrm.LookBudgetEnter(Sender: TObject);
begin
...
// showmessage(QDetailKODE_BEBAN.AsString);
 DMFrm.QLookBudgetKAS.SetVariable('ID_BAG',QDetailKODE_BEBAN.AsString+'%');

  DMFrm.QLookBudgetKAS.SetVariable('KDPERK',QDetailKD_PERK.AsString);
//   showmessage(QDetailKD_PERK.AsString);
{MAA}
  DMFrm.QLookBudgetKAS.Open;

end;

procedure TBKKFrm.LookBudgetUserButton2Click(Sender: TObject;
  LookupTable: TDataSet);

begin
{azmi}
    if InputQuery('Filter Berdasarkan Bagian','BAGIAN : ',kd_filter) then
    begin
        DMFrm.QLookBudget.DisableControls;
        DMFrm.QLookBudget.Close;
        DMFrm.QLookBudget.SetVariable('id_bag','%');
        DMFrm.QLookBudget.SetVariable('bagian','%'+kd_filter+'%');
...
procedure TBKKFrm.LookBudgetUserButton1Click(Sender: TObject;
  LookupTable: TDataSet);
begin
{azmi}
        DMFrm.QLookBudget.DisableControls;
        DMFrm.QLookBudget.Close;
        DMFrm.QLookBudget.SetVariable('id_bag','%');
        DMFrm.QLookBudget.SetVariable('bagian','%');
        DMFrm.QLookBudget.Open;
        DMFrm.QLookBudget.EnableControls;
{azmi}
end;

procedure TBKKFrm.BtnAmbilClick(Sender: TObject);
begin
  if (ENoNota.Text='') or (copy(ENoNota.Text,1,3)<>'BCK') then
        ...
        begin
          ...
          begin
            try
              ...
              QKas2.Close;
              QKas2.SetVariable('kd_cek_bg','BCK');
              QKas2.SetVariable('no_nota',ENoNota.Text);
              QKas2.Open;
              QKas2.EnableControls;
              QTransaksiKD_REKANAN.AsString:=QKas2KD_REKANAN.AsString;
              ...
              QAmbilNota.SetVariable('kd_cek_bg','BCK');
...
procedure TBKKFrm.LookBagianEnter(Sender: TObject);
begin
...
procedure TBKKFrm.BitBtn3Click(Sender: TObject);
var
  tbayar : string;
...
procedure TBKKFrm.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
inc(vrecord_ke);
...
procedure TBKKFrm.DetailBand1AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
inc(vnomer2);
...
procedure TBKKFrm.LookNotaEnter(Sender: TObject);
begin
  QNota.Close;
  QNota.SetVariable('kd_rekanan','%'+QTransaksiKD_REKANAN.AsString);
//  QNota.SetVariable('kd_perk_bayar','%');
...
procedure TBKKFrm.SummaryBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TBKKFrm.BitBtn5Click(Sender: TObject);
begin
...
procedure TBKKFrm.LookPerkiraanCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
...
procedure TBKKFrm.LookNotaPPH23Enter(Sender: TObject);
begin
  QNota_PPH23.Close;
  QNota_PPH23.Open;
end;

procedure TBKKFrm.LookNotaPPH23CloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if modified then
  begin
  QDetailNO_REG_HUTANG.AsInteger:=QNota_PPH23NO_REG_HUTANG.AsInteger;
  QDetailNO_NOTA_HP.AsString:= '-'{QNota_PPH23NO_NOTA.AsString};
  QDetailPPH23.AsFloat:=QNota_PPH23HT_PPH23.AsFloat;
  QDetailDEBET.AsFloat:=QNota_PPH23HT_PPH23.AsFloat;
  QDetailKD_PERK.AsString:=QNota_PPH23COA.AsString;
  QDetailKETERANGAN.AsString:=QDetailNAMA_PERKIRAAN.AsString+'** '+
  QNota_PPH23REKANAN.AsString+' Nota : '+QNota_PPH23NO_NOTA.AsString;
  QDetailKD_REKANAN.AsString:=QNota_PPH23KD_REKANAN.AsString;
  QDetailTANGGAL.AsDateTime:=QTransaksiTANGGAL.AsDateTime;
  QDetailKD_REKANAN.AsString:=QNota_PPH23KD_REKANAN.AsString;
  {QDetailHP_ORG.AsFloat:=QNotaNILAI.AsFloat;
  QDetailHP_TERBAYAR.AsFloat:=QNotaDIBAYAR.AsFloat;}
...
procedure TBKKFrm.LookNotaCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
if modified then
  begin
...
procedure TBKKFrm.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
 vmyberjalan:=0;
end;

procedure TBKKFrm.QDetailNewRecord(DataSet: TDataSet);
begin
...
procedure TBKKFrm.FormCreate(Sender: TObject);
begin
  vkd_perk2:=DMFrm.QHakPerkiraanKD_PERK.AsString;
end;

procedure TBKKFrm.LookPerkiraanEnter(Sender: TObject);
begin
  DMFrm.QPerkiraan.Open;
  DMFrm.QCash.Open;
end;

procedure TBKKFrm.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inc(vnomer);
...
procedure TBKKFrm.BitBtn_VerifClick(Sender: TObject);
var
vuser, vpwd : string;
vdatetime  : TDateTime;
begin
...
procedure TBKKFrm.PageHeaderBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  vfirst_page:=False;
end;

procedure TBKKFrm.BitBtn4Click(Sender: TObject);
begin
  QRLabel28.Caption:='PV';
  Quickrep3.Preview;
end;
